...
- **Relationship**: Many-to-one from Article/Video to Source, **not** one-to-one. `Source.find_or_create` deduplicates by (content, type, language), so two articles with identical content deliberately share one `source_id` — an original and a simplification that came out identical, or the same piece crawled twice. Production has 6465 such source_ids. Never assume `Article.query.filter_by(source_id=...)` returns at most one row, and be careful joining `Bookmark.source_id == Article.source_id`: that join fans out where a source has several articles.