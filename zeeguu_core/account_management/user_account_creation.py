...
from zeeguu_core.model import Cohort, User, Teacher, Language, UserLanguage
from zeeguu_core.model.user import CEFR_TO_DIFFICULTY_MAPPING


def valid_invite_code(invite_code):
    if zeeguu_core.app.config.get(
        "INVITATION_CODES"
    ) and invite_code in zeeguu_core.app.config.get("INVITATION_CODES"):
        return True
