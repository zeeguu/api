...
def past_exercises_for(user_id):
    ...
    for ex in q.all():
        ...
        if corrects_in_a_row:
            print(
                f"{ex.time.day}/{ex.time.month} {bookmark.meaning.origin.word}({bookmark.id}) {ex.outcome.outcome}:{corrects_in_a_row} < ({past})"
            )
        else:
            print(
                f"{ex.time.day}/{ex.time.month} {bookmark.meaning.origin.word}({bookmark.id}) {ex.outcome.outcome} < ({past})"
            )
    ...
    for bookmark in user.all_bookmarks():
        ...
        print(
            ...
            + f"{bookmark.id} "
            + f"{bookmark.meaning.origin.word} / {bookmark.meaning.translation.word}"
        )
