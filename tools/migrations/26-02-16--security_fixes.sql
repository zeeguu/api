...
-- 1. Expand unique_code.code column to support secure 64-character tokens (was 4 chars for 3-digit codes)
-- 2. Expand user.password column to support bcrypt hashes (60 chars)

-- Expand unique_code.code from VARCHAR(4) to VARCHAR(64) for secure tokens
ALTER TABLE unique_code MODIFY COLUMN code VARCHAR(64);
