...
def validate_before_examples(user_word: UserWord) -> Optional[UserWord]:
    """
    ...
    try:
        ...
        log(f"[VALIDATION] Skipping validation in prefetch: {e}")
        return user_word

    # Get context from preferred bookmark
    bookmark = user_word.preferred_bookmark
    if not bookmark:
        log(f"[VALIDATION] No preferred bookmark for {user_word.id}, skipping validation")
        return user_word

    context = bookmark.get_context()
    if not context:
        log(f"[VALIDATION] No context for {user_word.id}, skipping validation")
        return user_word
    ...
    if result.is_valid:
        ...
        user_word.update_fit_for_study(db.session)
        return user_word
    else:
        log(f"[VALIDATION] Translation needs fixing: {result.reason}")
        return _fix_bookmark_in_prefetch(user_word, bookmark, result)


def _fix_bookmark_in_prefetch(user_word: UserWord, bookmark, validation_result) -> Optional[UserWord]:
    """
    Fix bookmark with corrected word/translation during prefetch.
    ...
    if new_word == old_meaning.origin.content and new_translation == old_meaning.translation.content:
        ...
        db.session.add_all([old_meaning, user_word])
        db.session.commit()
        return None
    ...
    if new_meaning.id != old_meaning.id:
        ...
        transfer_learning_progress(db.session, old_user_word, new_user_word, bookmark)

        # Update bookmark to point to new user_word
        bookmark.user_word = new_user_word
        new_user_word.preferred_bookmark = bookmark
        db.session.add_all([bookmark, new_user_word])

        # Cleanup old user_word if orphaned
        cleanup_old_user_word(db.session, old_user_word, bookmark)
...
def generate_examples_for_user_word(
    ...
    validated_user_word = validate_before_examples(user_word)
...
def send_completion_notification(stats: dict, user: Optional[User] = None, send_email: bool = False):
    """
    Send email notification when batch generation completes.
    
    Args:
        stats: Dictionary with generation statistics
        user: Specific user if generation was for single user, None for all users
        send_email: If True, send the email notification
    """
    if not send_email:
        log("Email notification not requested, skipping")
        return
    ...
    try:
        ...
        # Create subject
        if user:
            scope = f"user {user.name} ({user.id})"
        else:
            scope = "all active users"
            
        subject = f"📝 Example Prefetch Complete - {scope}"
        
        # Create detailed body
        body = f"""Batch Example Generation Completed

Scope: {scope}
Started: {stats['start_time'].strftime('%Y-%m-%d %H:%M:%S')}
Completed: {stats['end_time'].strftime('%Y-%m-%d %H:%M:%S')}
Duration: {duration_minutes:.1f} minutes

Results:
- Words processed: {stats['processed']}/{stats['total_words']}
- Examples created: {stats['examples_created']}
- Errors encountered: {stats['errors']}
- Success rate: {(stats['processed'] - stats['errors']) / max(stats['processed'], 1) * 100:.1f}%
- First example ID: {stats['first_example_id'] or 'None'}
- Last example ID: {stats['last_example_id'] or 'None'}

Performance:
- Average time per word: {stats['duration_seconds'] / max(stats['processed'], 1):.2f} seconds
- Examples per minute: {stats['examples_created'] / max(duration_minutes, 0.1):.1f}

Status: {'✅ Success' if stats['errors'] == 0 else '⚠️ Completed with errors'}

---
Generated by Zeeguu Example Prefetch System
"""
        ...
        mailer.send()
        log(f"Sent completion notification email for batch generation")
        
    except Exception as e:
        # Don't fail the whole process if email fails
        log(f"Failed to send completion notification: {e}")
        import traceback
        log(f"Email error traceback: {traceback.format_exc()}")
...
def batch_generate_examples(
    ...
    Args:
        ...
        active_users_only: Only include words for users active in the last N days
        days_active_threshold: Number of days to consider for "active" users
        send_email: If True, send completion notification email
    ...
    for i, user_word in enumerate(user_words):
        try:
            ...
            if examples:
                ...
                # Track first and last example IDs
                for example in examples:
                    if first_example_id is None:
                        first_example_id = example.id
    ...
    send_completion_notification(stats, user, send_email)
...
def main():
    ...
    parser.add_argument(
        "--send-email",
        action="store_true",
        help="Send completion notification email when finished",
    )
