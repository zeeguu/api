...
def test_user_recommendations(user_email, show_details=True, count=20):
    ...
    print(f"ID: {user.id}")
    print(f"Learned Language: {user.learned_language.name if user.learned_language else 'None'}")
    ...
    if cohorts:
        for cohort in cohorts:
            ...
            print(f"    Language: {cohort.language.name if cohort.language else 'None'}")
            print(f"    Students: {cohort.get_current_student_count()}/{cohort.max_students}")
    ...
    try:
        ...
        if articles:
            ...
            for i, article in enumerate(articles, 1):
                ...
                if show_details:
                    print(f"     Published: {article.published_time.strftime('%Y-%m-%d %H:%M') if article.published_time else 'Unknown'}")
                    print(f"     Language: {article.language.name if article.language else 'Unknown'}")
                    ...
                    if topics:
                        topic_names = [t.title for t in topics] if hasattr(topics[0], 'title') else []
                        if topic_names:
    ...
    try:
        ...
        if videos:
            ...
            for i, video in enumerate(videos, 1):
                ...
                if show_details:
                    print(f"     Duration: {video.duration if hasattr(video, 'duration') else 'Unknown'}")
                    print(f"     Language: {video.language.name if video.language else 'Unknown'}")
                    print()
        else:
