...
def past_exercises_for(user_id):
  ...
  for ex in q.all():
      ...
      sorted_log = sorted(bookmark.exercise_log,
                          key=lambda x: datetime.datetime.strftime(x.time, "%Y-%m-%d"), reverse=True)
      ...
      if corrects_in_a_row:
          ...
          print(f"{ex.time.day}/{ex.time.month} {bookmark.origin.word}({bookmark.id}) {ex.outcome.outcome} < ({past})")
