...
def normalize_language_code(language_code: str) -> str:
    ...
    raise ValueError(f"Language {language_code} not supported. Available: {list(LANGUAGE_MAPPINGS.keys())}")
...
def get_voice_id(language_code: str, voice_name: str) -> str:
    ...
    if full_language_code not in VOICE_CONFIG:
        raise ValueError(f"Language {language_code} not supported. Available: {list(VOICE_CONFIG.keys())}")
    
    if voice_name not in VOICE_CONFIG[full_language_code]:
        available_voices = list(VOICE_CONFIG[full_language_code].keys())
        raise ValueError(f"Voice {voice_name} not available for {full_language_code}. Available: {available_voices}")
