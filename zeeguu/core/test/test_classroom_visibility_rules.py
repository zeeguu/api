...
Every row is one world, and what a student in it sees. The rules are
implemented in User.cohort_articles_for_user and the classroom_only feature
(user_feature_toggles._classroom_only); this file states them in one place and
lets the real code answer. (Which uploaded texts reach the feed and search is a
separate rule: zeeguu.core.classroom.hide_unshared_uploaded_texts.)
...
class ClassroomVisibilityRulesTest(ModelTestMixIn, TestCase):
    ...
    def _world(self, learning, classes):
        ...
        if learning:
            self._language(learning)  # set_learned_language looks the row up
