...
import zeeguu.core
from zeeguu.core.test.rules.user_exercise_session_rule import ExerciseSessionRule
...
class UserExerciseSessionTest(ModelTestMixIn, TestCase):
    def setUp(self):
        ...
        self.exercise_session_timeout = (
            UserExerciseSession.get_exercise_session_timeout()
        )
    ...
    def test__is_not_same_exercise_session(self):
        new_exercise_session = UserExerciseSession(
            self.ex_session1.user_id, datetime.now()
        )
    ...
    def test__update_last_use(self):
        ...
        updated_session = self.ex_session1._update_last_action_time(
            db_session, current_time
        )
