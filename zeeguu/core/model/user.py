...
class User(db.Model):
    ...
    def search_users(cls, current_user_id: int, term: str, limit: int = 20):
        ...
        if term:
            ...
            filters.append(cls.username.like(f"%{escaped}%", escape="\\"))  # case-insensitive partial match for username
            # The email column is only stored in lower case
            filters.append(cls.name == term)  # exact match for name
