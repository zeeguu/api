...
class ArticleLevelTitleContext(db.Model):
    """
    A context that is found in a per-level headline of an Article (the ``title``
    on ArticleLevelSummary). Anchors a bookmark to a SPECIFIC level's title so
    ...
    Separate from ArticleLevelSummaryContext even though both point at the same
    ArticleLevelSummary row: a level's title and its summary are two different
    ...
    def __repr__(self):
        return f"<ArticleLevelTitleContext als:{self.article_level_summary_id}, b:{self.bookmark_id}>"
    ...
    @classmethod
    def get_all_user_bookmarks_for_article_level_title(
