...
class UserLoginTest(APITestMixin, TestCase):
    ...
    def test_set_language(self, mock_invite_code):
        ...
        self.string_from_api_post(f'/add_user/{EMAIL}',
                                  dict(password=PASS,
                                       username=USERNAME))

        session = self.json_from_api_post(f'/session/{EMAIL_DIFFERENT_CASE}',
                                          dict(password="pass"))
        assert(int(session))
    ...
    def test_set_language_without_invite(self, mock_invite_code):
        ...
        result  = self.string_from_api_post(f'/add_user/{EMAIL}',
                                  dict(password=PASS,
                                       username=USERNAME))
