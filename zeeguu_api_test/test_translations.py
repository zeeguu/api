...
class TranslationTests(APITestMixin, TestCase):
    ...
    def test_get_possible_translations(self, mock_get_all_translations):
        ...
        MIN_CONTEXT, self.data["query"] = minimize_context(
            self.data["context"],
            self.data["from_lang_code"],
            self.data["word"])
        mock_get_all_translations.return_value = (
            MockTranslator({"klein": ["small", "little"],
                            "krassen": ["big"]}).translate(self.data["query"]))
        ...
        form_data = dict(
            ...
            word=self.data["word"])
        with patch(
            "zeeguu_api.api.translate_and_bookmark.minimize_context") as (
                mock_minimize_context):
            mock_minimize_context.return_value = (
                MIN_CONTEXT, self.data["query"])
            alternatives = self.json_from_api_post(self.api_endpoint,
                                                   form_data)
        ...
        assert (first_alternative["likelihood"] >=
                second_alternative["likelihood"])
    ...
    def test_get_translation_where_gslobe_fails_but_translate_succeeds(
        ...
        mock_get_all_translations.return_value = (
            MockTranslator({
                "klein": ["small"],
                "krassen": ["big", "extreme"]}).translate(self.data["query"]))
    ...
    def test_translate_and_bookmark(self, mock_get_all_translations):
        ...
        NEW_MIN_CONTEXT, new_data["query"] = minimize_context(
            new_data["context"],
            new_data["from_lang_code"],
            new_data["word"])
        mock_translator = MockTranslator({"Die": ["The"], "kleine": ["small"]})
        new_mock_translator = MockTranslator(
            {"Die": ["The"], "kleine": ["small"]})
        ...
        with patch(
            "zeeguu_api.api.translate_and_bookmark.minimize_context") as (
                mock_minimize_context):
            mock_minimize_context.return_value = (
                NEW_MIN_CONTEXT, new_data["query"])
