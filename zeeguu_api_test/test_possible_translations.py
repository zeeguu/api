...
class TestPossibleTranslations(APITestMixin, TestCase):
    ...
    def test_get_possible_translations(self, mock_get_all_translations):
        ...
        mock_get_all_translations.return_value = (
            MockTranslator({"schon": ["nice"]}).translate(QUERY))
        ...
        with patch(
            ...
            translations = self.json_from_api_post(
                API_ENDPOINT, dict(context=CONTEXT,
                                   url=VERY_EASY_STORY_URL,
                                   word=WORD,
                                   title=TITLE))
            mock_minimize_context.assert_called_once_with(
                CONTEXT, FROM_LANG_CODE, WORD)

        data = {"from_lang_code": FROM_LANG_CODE,
    ...
    def test_get_possible_translations2(self, mock_get_all_translations):
        CONTEXT = ("Da sich nicht eindeutig erkennen lässt, "
        ...
        MIN_CONTEXT, QUERY = minimize_context(CONTEXT, FROM_LANG_CODE, WORD)
        mock_get_all_translations.return_value = (
            MockTranslator({"glatzköpfig": ["gifpökztalg"]}).translate(QUERY))
