...
class FeedTests(APITestMixin, TestCase):
    ...
    def test_start_following_feed(self):
        ...
        # Then
        followed_feed_ids = [each['id'] for each in self.json_from_api_get(f"/{FOLLOWED_FEEDS}")]
    ...
    def test_non_subscribed_feeds(self):
        ...
        non_subscribed_feeds = self.json_from_api_get(f"{RECOMMENDED_FEEDS}/{lang_code}")
